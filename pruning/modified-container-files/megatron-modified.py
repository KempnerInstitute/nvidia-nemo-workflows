...
class _DynamicGPTModel(DynamicModule):
    ...
    def _export_drop_layers(self) -> None:
        ...
        visualize_layer_pruning(layer_scores, num_layers_keep=num_layers_hp.active)
...
def visualize_layer_pruning(layer_scores: dict, num_layers_keep: int, output_path: str = "pruned_layers.png"):
    """
    Visualize layer importance scores and highlight pruned layers.

    Args:
        layer_scores (dict): Dictionary mapping layer numbers to importance scores.
        num_layers_keep (int): Number of layers to keep (the rest will be marked as pruned).
        output_path (str): Where to save the visualization image.
    """

    # Sort layers by score descending
    sorted_items = sorted(layer_scores.items(), key=lambda x: x[1], reverse=True)
    kept = sorted_items[:num_layers_keep]
    pruned = sorted_items[num_layers_keep:]

    all_layers = kept + pruned
    layer_numbers = [layer for layer, _ in all_layers]
    scores = [score for _, score in all_layers]
    colors = ['green' if i < num_layers_keep else 'red' for i in range(len(all_layers))]

    # Plot
    plt.figure(figsize=(12, 6))
    bars = plt.bar(layer_numbers, scores, color=colors)
    plt.xlabel("Layer Number")
    plt.ylabel("Importance Score")
    plt.title("Layer Importance Scores (Green = Kept, Red = Pruned)")
    plt.grid(axis="y", linestyle="--", alpha=0.5)

    # Annotate bars with values
    for bar, score in zip(bars, scores):
        height = bar.get_height()
        plt.text(bar.get_x() + bar.get_width()/2, height, f'{score:.2f}', ha='center', va='bottom', fontsize=8)

    # Save to file
    plt.tight_layout()
    plt.savefig(output_path)
    plt.close()
    print(f"Layer pruning visualization saved to {output_path}")
